...
## Status
**37 of 38 resolved — one PARKED item, nothing scheduled.** Every original audit item (n.1–n.15 · e.1–e.6 · a.1–a.8) is closed, and so is everything spun out during testing: **n.16** (the named vault base directory), **n.17** + **e.9** (the **creation language now reaches all three apps** — one engine, `#` `$` `/` `{…}` on tasks, notes *and* events), **e.10** (the "all calendars" nav row), **a.9** (the glyph list is the enforced source of truth — 40 glyphs, an off-list one is a 400), and **e.8** (the agenda's `J`/`K` + `H`/`L` shift layer). **n.11** is closed as a non-issue (the link chips are keyboard-navigable). **e.12** (the guided recurrence builder, lifted from the task drawer onto the event drawer) is built and ready to test. The only thing not resolved is **e.11** (yearly/annual recurrence), now **parked** — the builder scratches the itch, and a monthly-every-12 covers a birthday until/unless the user wants the grammar expanded.
...
| 10 | ✅ **e.12** — the guided recurrence builder (chips + inputs) lifted onto the event drawer; the `<recurrence-builder>` component was already factored out · ⏸ **e.11** — yearly recurrence parked (the builder + monthly-every-12 cover it) |
...
**Outstanding:** **e.12** ⏳ built, awaiting your smoke test · **e.11** ⏸ parked (yearly recurrence — the builder made it optional). Nothing else open.
...
**e.12 — Feature: the guided recurrence builder on the event drawer.** The event drawer's recurrence was a **raw text field** (you had to know the syntax); the task drawer has a chips-and-number-inputs **builder** that generates the syntax for you, with a live "next 3 occurrences" preview. *(User: "steal the UI and put it on the calendar rh drawer… 9 times out of 10 I'm creating a one-off recurrence on whatever calendar item I'm working on.")*
> **The lift was tiny, because the builder is already a factored-out component.** It's not inline in task-detail — it's `window.RecurrenceBuilder` (`frontend/js/recurrence-builder.js`), self-contained: props `modelValue` / `anchor` / `active`, emits `update:modelValue`, round-trips through `Rec.parse`/`Rec.stringify`. So this was **mount-and-wire**, all in `event-editor.js`: the flat input → `<recurrence-builder :model-value="f.recurrence" :anchor="f.date" :active="recurActive">`, the `recurrence` kbRow → `type:'static'`, and a `recurActive` sub-pane branch prepended to the existing `kbDelegate` — the exact pattern task-detail uses (`i` descends in, `esc`/`h`-at-edge/`k`-off-top pops out). Because it writes back to `f.recurrence`, the dirty-guard and save path needed **zero** change, and the chips' CSS came free (`.detail`-scoped). **Bonus:** editing an existing recurring event now shows its rule in the chips (the builder hydrates via `Rec.parse`), and the events grid already expands the series (`events.ts:220`).
> **✅ READY TO TEST**
> **How to test:** open a new-event drawer with **`I`** on the calendar (or click a day). `j`/`k` to **recurrence** (reads "— i to edit") → `i` descends into the builder on the frequency chips. `weekly` → weekday chips appear; `h`/`l` across them, space toggles `mon`+`wed`; the syntax line shows `weekly on mon,wed` and the preview shows the next 3 dates. `esc` pops back out. Save → reopen → the chips **rehydrate**. Edit a seeded recurring event → its rule shows in chips, not raw text. The raw syntax box still works as an escape hatch. Dirty-guard: change the frequency, `esc` out, close → "discard changes?" fires.
...
>
> **⏸ PARKED (user's call).** With **e.12** shipping the guided builder, the itch is scratched a different way: *"If I need [yearly] I'll make a monthly recurring item that repeats every 12 months… If it bothers me I'll expand the syntax and chip."* So the yearly grammar stays a documented future option, not scheduled work — and when it comes it's this one clean branch across the shared module (parse · matches · stringify · a new `yearly` chip in `recurrence-builder.js`), with Feb 29 the only real decision.
