...
## Status
**Done — every item resolved.** (Yearly recurrence, the one deferred idea, was moved to the prod tdx tracker at the user's request; it was never a keyboard-nav gap.) Every original audit item (n.1–n.15 · e.1–e.6 · a.1–a.8) is closed, and so is everything spun out during testing: **n.16** (the named vault base directory), **n.17** + **e.9** (the **creation language now reaches all three apps** — one engine, `#` `$` `/` `{…}` on tasks, notes *and* events), **e.10** (the "all calendars" nav row), **a.9** (the glyph list is the enforced source of truth — 40 glyphs, an off-list one is a 400), and **e.8** (the agenda's `J`/`K` + `H`/`L` shift layer). **n.11** is closed as a non-issue (the link chips are keyboard-navigable). **e.12** (the guided recurrence builder, lifted from the task drawer onto the event drawer) is user-tested and closed — the last item. The full keyboard + creation-language surface is complete across tasks, notes and events.
...
| 10 | ✅ **e.12** — the guided recurrence builder (chips + inputs) lifted onto the event drawer; the `<recurrence-builder>` component was already factored out |
...
**Outstanding:** none. The keyboard audit is closed. (Yearly recurrence lives in the prod tdx tracker.)
...
*Nothing — every events item is resolved.*
...
> **✅ VERIFIED & RESOLVED** — user-tested (the recurrence builder passed).
