...
function readState(userId) {
  ...
  const savedQueries = db
    .prepare('SELECT id, name, glyph, query, system, color FROM saved_queries WHERE user_id = ? ORDER BY position, id')
    .all(userId)
    .map((s) => ({
      id: s.id,
      name: s.name,
...
function writeState(userId, snapshot, expectedVersion) {
  ...
  const insSaved = db.prepare(
    'INSERT INTO saved_queries (user_id, id, name, glyph, query, system, color, position) VALUES (?, ?, ?, ?, ?, ?, ?, ?)'
  );
  ...
  const apply = db.transaction(() => {
    ...
    savedQueries.forEach((s, i) => {
      insSaved.run(userId, s.id, s.name, s.glyph, s.query, s.system ? 1 : 0, s.color ?? null, i);
    });
